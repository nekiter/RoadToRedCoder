...
void checkMagazine(vector<string> magazine, vector<string> note) {
    ...
    {
        if(notesMap.find(note[i]) == notesMap.end()){
            //Failed to find word on map
            notesMap[note[i]] = 1;
        } else {
            notesMap[note[i]]++;
        }
    ...
    for ( const auto &myPair : notesMap ) {
        if(magazineMap.find(myPair.first) == notesMap.end()){
            cout << "No" << endl;
            return;
        } else {
            if( magazineMap[myPair.first] < myPair.second){
                cout << "No" << endl;
                return;
            }
        }
